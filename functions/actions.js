...
function actions() {
    ...
    return new Promise(function (resolve, reject) {
      if (data.output.hasOwnProperty('actions')) {
        data.output.actions.forEach(function (action) {
          ...
          else if (action.name === 'ValidateEmail') {
            console.log('1st stage successfull')
            endPoint = '/bank/sendotp';
            value = action.parameters.emailid;
            console.log(`value is ${value}`)
            sendType = action.result_variable;
            console.log(`sendType is ${sendType}`)
            sendEmail(endPoint, value, sendType, sessionId, resolve, reject);
          }
        });
      } 
  ...
  function sendEmail(endPoint, value, sendType, sessionId, resolve, reject) {
    // construct the endpoint based on which client action
    var parameterizedEndpoint = endPoint + '?value=' + value;
    console.log(`parameterizedEndpoint endpoint is ${parameterizedEndpoint}`)
    http.get({
  ...
  function sendMessage(res, value, sendType, sessionId, resolve, reject) {
    ...
    if (sendType === 'input.text') {
      ...
      console.log(`payloadToWatson : ${res.result}`)
